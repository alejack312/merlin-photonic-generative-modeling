...
## Exact reference vs. mechanism-removing control (GATE-02, added 2026-09-05)

A classical reproduction of a measurement is not automatically evidence the circuit's mechanism is absent from it. Check which mathematical object the reproduction actually varies: if it retains the same `q(theta)` the real circuit computes (an exact classical formula for the same construction — the "classically trainable by construction" case any IQP circuit already has via Van den Nest's cosine formula), it proves the measurement is classically reproducible, not that removing the circuit changes nothing. A genuine mechanism-removing control has to change what's being measured (e.g. a fixed reference distribution with no theta-dependence at all), not just how it's computed. This gate exists because v3.1's own correction made exactly this mistake: `test_null_results.py`'s `_product_q`/`_mixed_q` compute the literal same `q(theta)` as the real circuit, and the correction's "cannot be attributed to this circuit's loss landscape" conclusion outran what that proved. Full note: `.Codex/learnings/2026-09-05-reference-controls.md`.

## Classifier/verdict functions need adversarial tests before they back a claim (GATE-03, added 2026-09-05)

Before any function that outputs a scientific verdict label (plateau/no-plateau, pass/fail, survives/disappears) is trusted for a finding, it needs test cases for its own documented edge behavior — not just the cases it was designed around. Concretely: sign conventions on a fitted curve's parameters (does the check actually verify the direction it claims to?), what happens when only one of two competing models converges, and boundary/degenerate inputs. This gate exists because `fit_verdict_to_plateau_label` and `classify_survival` both checked only `b > 0` in `a*exp(-b*n)+c` — whether the curve actually decays depends on `a`'s sign too — and mislabeled a strictly increasing sequence "plateau," undetected until an independent audit's adversarial probe. `fit_and_compare` separately contradicted its own documented single-convergence contract. Neither had a test for the case that broke it.
