...
- Full suite: `venv/Scripts/python.exe -m pytest -q` → `724 passed, 1 skipped` with a writable `PCVL_PERSISTENT_PATH`.
- Focused correction suite → `186 passed, 1 skipped`; explicit sibling integration → `36 passed`.
- Artifact validation → `579` JSON files, `72` JSONL rows, `19` payload hashes, `0` failures.
...
- [Repaired physical controls](../results/v4_tcdp/deploy/physical_control_manifest_20260909_final3.json)
- [Repaired spatial ring evaluation](../results/v4_tcdp/deploy/registered_v3_ring_photonic_spatial_n4_seed0_smoke_20260909_final3.json)
- [Repaired Hamming ring evaluation](../results/v4_tcdp/deploy/registered_v3_ring_photonic_hamming_n4_seed0_smoke_20260909_final3.json)
...
- A fresh direct full-Fock manifest records final-only and intermediate projections, absolute accepted mass, conditional distributions and the corrected shared-gate comparison (`1.942890293094024e-16` TVD with unchanged accepted mass). The fixed-photon physical scope passes; it does not certify larger-n full-Fock behavior or multiphoton source models.
...
- Fixed-photon `g2=0` Perceval controls pass for n=2/n=3 no-gate, single-gate-with-bystander, and shared-gate cases. Absolute accepted mass and conditional output are separate. After coherent-readout repair, the registered shared-gate final-only/intermediate conditional TVD is `1.942890293094024e-16` with unchanged accepted mass; final-only remains the supported boundary by declared scope, without a general theorem.
