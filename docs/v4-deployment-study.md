...
The logical-1 optical phase is recorded as PS(-2t). The full-Fock adapter prepares `(1,0)` per pair but decodes output `(0,1)` as logical zero and `(1,0)` as logical one; the input and output conventions must not be conflated. Pair angles use the 63-key catalog 0.1*j, j=0..62, with circular-nearest lookup. The wrapped CP key and the lifted compensation angle are separate fields; winding is never discarded.
