...
> **Documentation reconciliation, 2026-09-10.** This is the v3 study record. Its measured circuit families are classically sampleable. The legacy quantity called TVD on subnormalized vectors is a half-L1 diagnostic, not TVD between two normalized distributions; conditional quality and absolute success must be separated. Historical literature verdicts and illustrative loss thresholds are not hardness evidence. See [the corrected synthesis](technical-findings.md) and [v4 optical boundary](v4-bounded-release.md).
...
**What the data establishes:** Conditional shape is unchanged in the tested uniform-loss model. The tested lossless families are already classically sampleable, so this establishes no preserved sampling hardness. The herald-conditioned surviving mass above and the absolute probability of a usable source attempt are different quantities. The table below uses the resource model `P(usable)=eta^(n+2k)*(2/27)^k`, assuming two real ancilla photons and the ideal success factor for each of k heralded CZ gates. Its k>=2 entries assume composable independent success factors; they are not multi-gate measurements or a proof of that factorization. The reciprocal is expected source attempts per usable outcome. No unconditional-output complexity result follows.

**Throughput (expected samples needed per usable shot), the figure this correction leads with:**

| n (data qubits) | k=0 (analytic estimate) | k=1 (analytic estimate) | k=2 (extrapolated, not measured) | k=3 (extrapolated, not measured) |
|---|---|---|---|---|
...
Herbst et al. discuss anticoncentration and average trainability. This study does not test that theorem: its conditioned shape is invariant in the implemented uniform-loss setting and its circuit families are classically sampleable. The former characterization as a general loss-driven co-occurrence prediction is withdrawn. See [the current literature scope](iqp-lit-scoping.md).
