...
- **Oh, "Classical simulability of constant-depth linear-optical circuits with noise," npj Quantum Information (2025), arXiv:2406.08086.** Derives a percolation threshold in loss *and* partial distinguishability jointly, above which single-photon linear-optical circuits decompose into small, classically-simulable pieces. This is a loss-native hardness-adjacent theorem — no eta-to-depolarizing-rate translation needed — that `docs/hardness-under-loss-study.md`'s HARD-04 section states it could not find; it should be positioned there once read in full, not just noted here.
