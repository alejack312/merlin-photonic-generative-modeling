...
**An external audit found, and this project independently re-verified, that the "exp" verdicts below do not measure the circuit.** At `sigma=0.03` and `sigma=0.1` (the bandwidths where the "exp" verdict appears), `sigma` is smaller than the target grid's bin spacing at every swept `n` (`target_grid.bin_spacing`: 1.2, 0.4, 0.4, 0.17, 0.17 for `n=2..6`). This makes the Gaussian kernel numerically the identity matrix (`tests/v3_correction/test_null_results.py::test_kernel_is_identity_below_bin_spacing`), so `MMD²` reduces to the plain squared-L2 distance between the circuit's output vector and the fixed target — no kernel smoothing, no distributional comparison in the sense the metric was designed for.

Given that, the measured gradient-variance-vs-`n` curve is reproduced with **no photonics at all**: a ten-line closed-form model (`weight1` — a pure per-qubit product distribution; `mixed` — that same product with one pair additionally coupled at the fixed `pi/4` angle, exactly matching this sweep's own circuit) run through this project's own `target_grid`/`mmd_exact` code reproduces the shipped curve. This is now a standing regression test (`tests/v3_correction/test_null_results.py`), not a one-off check.

**This is not a barren plateau, for a structural reason as well as an empirical one:** `weight1` has no entangling gate. Its output is a pure product distribution (already established in `docs/hardness-under-loss-study.md`'s HARD scope precondition), and a per-qubit gradient on a product distribution cannot exhibit the collective, whole-register concentration a barren plateau describes. The exponential *shape* in the curve is real — it's the L2-squared-distance-of-a-product-distribution's own well-known `~2^-n` scaling, not evidence about the circuit's loss landscape.

**This also resolves TRAIN-09's own puzzle.** The bandwidth-sensitivity sweep below found the "exp" signature present at `sigma in {0.03, 0.1}`, gone at `sigma in {0.3, 1.0}`, and back (non-monotonically) at `sigma in {3.0, 9.0}`. That pattern is exactly what this mechanism predicts: the kernel is the identity at low `sigma` (artifact present), genuinely smooths at intermediate `sigma` (artifact gone, and no other signal replaces it), and at high `sigma` the kernel saturates toward a different degenerate limit (a new, different artifact, not a returning trainability signal). No property of the circuit needs to change for any of this to happen.

**The Rudolph et al. literature-comparison row (below) is corrected accordingly**: this project's original fixed `SIGMA=0.1` is not merely "in the risk regime" their Theorem 2 flags — the identity-kernel mechanism confirmed here *is* the mechanism their theorem describes, not a directionally-similar but mechanistically-distinct case.

**What survives this correction:** `docs/iqp-baseline.md`'s Mhiri et al. and Recio-Armengol et al. rows, and TRAIN-10's negative result (data-dependent init did not resolve `small_angle`), are untouched — they don't depend on the sigma-below-bin-spacing artifact. The tables and fitted curves below are kept as a documented pipeline artifact, not deleted, per this project's standing correction convention; read them as "what the identity-kernel L2 metric on a product distribution looks like," not as a trainability result.
