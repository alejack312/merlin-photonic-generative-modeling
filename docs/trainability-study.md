...
> **Documentation reconciliation, 2026-09-10.** This is the v3 study record, including superseded interpretations and dated corrections. The current conclusion is that the headline curves do not establish a circuit-induced barren plateau or verify a literature theorem. The original tables are retained for auditability. Read [the corrected synthesis](technical-findings.md) and [current kernel/theorem conventions](iqp-baseline.md) before using the historical verdict tables. v4 is a separate classical pipeline.
...
**An external audit found, and this project independently re-verified, that the "exp" verdicts below do not measure the circuit.** At `sigma=0.03` and `sigma=0.1` (the bandwidths where the "exp" verdict appears), the Gaussian kernel is **numerically the identity matrix only for `n<=4`** (`target_grid.bin_spacing`: 1.2, 0.4 at n=2,3,4 — max off-diagonal kernel entry 3.4e-4, genuinely negligible). **This document's first version of this correction wrongly claimed the same held "at every swept n" through n=6.** At `n=5,6` the bin spacing shrinks to 0.171, and the nearest-neighbor off-diagonal kernel entry is `exp(-0.171^2 / (2*0.1^2)) = 0.230` — a real, non-negligible off-diagonal term, independently confirmed by direct computation (`gaussian_kernel_matrix_np`), not the identity. `tests/v3_correction/test_null_results.py::test_kernel_is_identity_below_bin_spacing` only ever checked `n<=4`, correctly. The prose here previously extended that finding further than the test actually established, the exact kind of gap this whole correction exists to close.

What survives, precisely: the regression test that reproduces the shipped curve (`closed_form_gradient_variance`) uses the **real** kernel throughout — it never assumed identity — and still reproduces the shipped `n=2..6` curve within a documented tolerance (`rel=0.2`, once the null model pools `pick_tracked_indices(n, 3)` the way the real sweep does rather than differentiating only `theta[0]` — an earlier `rel=0.5` had been widened to paper over that pooling bug rather than reflecting a genuine `n=5,6` fit difference). So the "no photonics needed" claim survives at every `n`, but the *mechanism* is more precisely: exact L2-on-a-product-distribution for `n<=4`, and a partially-kernel-smoothed version of the same classical null computation for `n=5,6` — not literally identical to L2 there. Both are reproduced with **no photonics at all**, using this project's own `target_grid`/`mmd_exact` code and a product-distribution model (`weight1`) or that same product plus one fixed-`pi/4`-coupled pair (`mixed`) — this is now a standing regression test (`tests/v3_correction/test_null_results.py`), not a one-off check.

**This is not evidence of a barren plateau, for a structural reason as well as an empirical one — stated at the strength this actually supports.** `weight1` has no entangling gate. Its output is a pure product distribution. The measured decay is fully reproduced by a model with no photonic circuit evaluation, so these data do not isolate a circuit-landscape contribution. They support the narrower conclusion that this exponential shape is reproduced by the null model's own `~2^-n`-type scaling; they do not prove that the circuit's loss landscape has no effect.
...
**The Rudolph et al. literature-comparison row (below) is corrected accordingly, at `n<=4` where the identity-kernel mechanism is exact**: this project's original fixed `SIGMA=0.1` is not merely "in the risk regime" their Theorem 2 flags — the identity-kernel mechanism confirmed here *is* the mechanism their theorem describes at those sizes. At `n=5,6`, the correction is weaker but still real: the shipped curve is fully reproduced by the classical null model for these tested circuit scopes, without establishing a circuit-independent result.

**What survives this correction:** `docs/iqp-baseline.md`'s Mhiri et al. and Recio-Armengol et al. rows, and TRAIN-10's deterministic-initialization diagnostic, are untouched — they don't depend on this artifact. The tables and fitted curves below are kept as a documented pipeline artifact, not deleted, per this project's standing correction convention. Read them as "what the tested classical null model of this loss reproduces," not as a trainability result.

### Correction (2026-09-03) — the gradient-computation method itself was chosen without asking what an IQP model actually needs

An independent Fable 5.1 review of the correction above found a second dimension to the same mistake: not just that the "exp" verdict is an artifact, but that the method used to compute the gradient in the first place was never checked against an alternative this project's own prior work already had working.

**The mechanism.** Van den Nest's cosine formula makes `<Z_a>` for an IQP circuit a classical expectation, `E_{z~Uniform}[cos(Phi(theta, z, a))]` — no circuit, no simulator, just a closed-form average over uniform bitstrings. Consequence: training an IQP Born machine never needs a quantum circuit or a photonic simulator at all. Only *sampling* from the trained model does. This project's own `docs/iqp-baseline.md` already names and cites this formula, and its attribution was independently confirmed on 2026-08-17 via Recio-Armengol et al. (arXiv:2503.02934), whose Proposition 1 derives the identical formula and cites den Nest (2010). The formula was in this project's own literature notes the whole time.

**The prior art was mine.** My own sibling project (`iqp-mmd-barren-plateau`, `docs/technical/learning-task.md` and `docs/technical/mmd-gaussian-fourier.md`) already implements exactly this: it trains an IQP Born machine entirely classically via the cosine formula, using a **Hamming-distance** kernel with bandwidth scaled per Rudolph et al. (arXiv:2305.02881) as `sigma in Theta(sqrt(n))`, giving `tau = tanh(1/(4*sigma^2)) ~ 1/n`. Its quantum circuit is only ever run to sample after training. That sibling project is already this study's own source for the MMD² methodology and for the Rudolph et al. baseline cited above — which is what makes the omission notable rather than merely unlucky: the working alternative was one repo away.

**What this study did instead, and why the stated reason was insufficient.** This document's own Methodology section states the true reason parameter-shift-on-Perceval was used: MerLin `QuantumLayer` autograd is structurally unavailable for this project's polarization-annotated `BasicState`s. That is correct. It establishes that *MerLin autograd* was unavailable. It does not establish that *parameter-shift executed twice per parameter directly on the Perceval-simulated circuit* was the only remaining option — concretely, this study used a **fixed** `sigma=0.1` and a **Euclidean-distance** kernel over 2D spatial target-grid bins, not the Hamming/Fourier-mode kernel the sibling project and Rudolph et al.'s own theorem are built on. An IQP circuit's gradient is obtainable without touching a circuit simulator at all, via the mechanism above. That question — what gradient does an IQP model actually need? — was never asked before the method was chosen. `docs/iqp-baseline.md` had already flagged the fixed, not-n-scaled bandwidth as "a live methodological question for Phase 17's already-shipped result, not resolved by this doc alone," written before v3.0 shipped. That flag was never acted on. The gap was visible in this repo's own notes.

**Scope of this addition.** No number, curve, closed form, fitted parameter, or verdict anywhere in this document changes because of this. The identity-kernel mechanism above, the regression tests in `tests/v3_correction/test_null_results.py`, and the corrected Rudolph literature-table row all stand exactly as written. This corrects how the method choice is framed, not any result. What is deliberately **not** claimed: whether a classical trainer would have produced a different verdict at this project's n range has not been checked and is not asserted here.
...
**Corrected conclusion:** The fitted labels vary with bandwidth. The low-bandwidth null reproduces the original headline curves; the untested large-bandwidth recurrence is not evidence of a distinct circuit effect. These fits do not establish an asymptotic barren plateau.
...
The data-dependent initialization diagnostic did **not** produce a clearer (non-inconclusive) verdict than `small_angle` in either generator scope: both `weight1/data_dependent` and `mixed/data_dependent` remain "inconclusive." Because TRAIN-10 used one deterministic parameter vector per system size (`n_draws=1`), its spread across coordinates is not the same statistic as the random-draw variance used for `small_angle`; this is a deterministic-initialization diagnostic, not a matched negative test of the literature-sourced alternative-init hypothesis.
...
**What this cross-check establishes:** A separate implementation can expose convention or evaluation differences. If two ideal encodings realize the same parameterized logical unitary, use the same target and loss, and condition identically, their logical probabilities and corresponding gradients must agree. A discrepancy requires checking those contracts, finite-sample effects and implementation errors before attributing it to the physical encoding.
...
- **Rudolph et al. (arXiv:2305.02881, Theorem 2)**: **Correction (2026-09-03): narrowed after an independent audit.** The row below is the pre-correction text, kept for the record. See the dated correction at the top of Results. This project's fixed `SIGMA=0.1` is not merely in the risk regime Theorem 2 flags — for the tested `n<=4` cases, the identity-kernel-on-a-product-distribution mechanism verified in `tests/v3_correction/test_null_results.py` matches the paper's constant-bandwidth argument, and the shipped curve is reproduced by the corresponding classical null model without photonic evaluation. The paper's bodyness decomposition assumes a bitstring-Hamming-distance kernel rather than this project's Euclidean-distance kernel, so the *specific formula* does not transfer. At `n=5,6`, the null-model reproduction remains evidence about these tested circuit scopes; it does not establish that fixed bandwidth forces concentration regardless of circuit structure.
...
- **Mhiri et al. (arXiv:2502.07889): not tested.** Commutativity among IQP generators does not imply commutativity with the measured observable or input state. The inconclusive small-angle rows do not independently validate a theorem about those other conditions.
- **Recio-Armengol et al. (arXiv:2503.02934): not independently tested by these rows.** The original uniform-init headline is reproduced by the classical null. TRAIN-10 uses one deterministic parameter vector per n, so it is not a variance ensemble or a matched test of the initialization claim. See [the current baseline](iqp-baseline.md) for the classical estimation boundary.
- **Herbst et al. (arXiv:2512.24801)**: this baseline's substantive content is the cross-reference note below. **Verdict: see Cross-reference note below.** TRAIN's own zero-loss data shows a bandwidth-sensitive gradient-concentration signature for `uniform` init, while the null-model reproduction does not isolate a circuit-landscape effect. TRAIN never varies loss and so cannot itself confirm or refute the paper's eta-dependent co-occurrence prediction.
...
**What TRAIN’s own dataset cannot do:** The small-n grid and initialization study does not establish an asymptotic concentration theorem. Its headline curves are reproduced by the null described above. Herbst et al. discuss anticoncentration and average trainability; describing that result as specifically an eta-axis prediction was too narrow. This dataset does not test its hypotheses or conclusions.
