...
This report records the 22 bounded ring comparison cells and eight available sibling comparison cells. See [the release result tables](v4-bounded-release.md) for target fit, support validity and expected sampling cost. It is an artifact-backed control, not evidence of a photonic learning advantage. The raw arm is the exact NumPy IQP evaluator; the primary compiled arm uses the catalog alpha-key quantization through absolute-probability ideal CP maps; the deployed arm uses the same compiled parameters and the fixed-photon final-only CP-map composition boundary validated by the dedicated n=2/n=3 Perceval controls. An explicit unquantized compiled arm is retained as `ideal-unquantized-control`. The deployed arm remains qualified as a fixed-photon final-only model-derived/reference arm; it is not a hardware result, multiphoton result, or intermediate-projection result.

## Matching contract

Both profiles use the same frozen train split, generator, final theta, MSB-first bit order, nominal 20,000-accepted-outcome cost budget (not collected shots), and Gaussian-on-Hamming kernel `exp(-H/(2 sigma^2))` with `sigma=.5*sqrt(n)`. Both ring profiles report spatial and Hamming MMD separately. Raw, compiled, and deployed vectors are stored separately; acceptance is reported separately from conditional distribution quality.

## Historical smoke artifacts
