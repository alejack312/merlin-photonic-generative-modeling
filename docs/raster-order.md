...
**Claude's Correction (2026-07-29): a bigger hole than either of the above.** At first I thought the "the circuit naturally produces smooth neighbor-to-neighbor outputs" line above was just a fact about the physics, safe to state flatly and build the whole argument on. But I realized that's an assumption I never actually tested, and it's the assumption the *entire* mechanism story depends on, not a side detail. Here's why it matters more than it looks: MMD² doesn't care about list order at all, as long as you shuffle the target, the circuit's output, and the distance table between them all the same way: reorder everything consistently and the loss number doesn't move. So "we made the target's list less scrambled" cannot, by itself, be why the score got better; a scoring function that's blind to order can't reward you for tidying up the order. The *only* way reordering the target's tiles helps is if the circuit itself has some real habit of moving neighboring slot-numbers together when its dials get nudged, and lining that habit up with the target's new tidy layout is what would actually produce the improvement. I asserted that habit exists. I never ran the check that would confirm it (e.g., nudge the circuit's parameters slightly and see whether list-neighbors under our ordering move together more than a random pair of slots would). Until that check runs, "the circuit's natural tendency to be smooth stops fighting the target shape" is a plausible story, not a demonstrated one, and the genuinely boring alternative (the loss's bandwidth, sigma=0.1, was tuned for the *old* 400-bin layout and never re-checked after this fix changed the grid to 462 bins; at that bandwidth the two rings are only 61% similar to each other, but the *empty gap between them* is 88% similar to either ring, meaning the loss barely penalizes gap-filling either way) hasn't been ruled out as the real cause of the ring_mass bump.