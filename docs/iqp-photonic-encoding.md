...
> **Documentation reconciliation, 2026-09-10.** This preserves the v2/v3 derivation and owner attempts. Logical gate identities and single-gate checks do not by themselves prove composed, final-only optical projection equivalence. v4 uses explicit dual-rail conventions and qualified small-circuit controls; larger analytic map comparisons are not direct optical validation. See [deployment contracts](v4-deployment-study.md) and [the bounded release](v4-bounded-release.md). Historical novelty claims are superseded by [the current literature scope](iqp-lit-scoping.md).

**Correction (2026-09-03) — the novelty claim below is overstated.** An external audit pointed out, correctly, that dual-rail photons plus heralded linear-optical gates (Knill, Laflamme & Milburn, 2001, arXiv:quant-ph/0110144 — the same construction `heralded_cz` implements) already let *any* qubit circuit, IQP included, be realized in linear optics. That KLM-universality fact is not something this project's literature search left open; it's a 2001 result this project's own `dual_rail.py` module demonstrates directly. This document's actual contribution is narrower: the specific polarization-encoding realization (which wave-plate angle realizes which gate, the exact port/bit convention, the herald-success bookkeeping, checked to floating-point precision at n=2-3) is real, checked engineering work — but "checked" and "original" are different claims, and this project's own literature search (`docs/iqp-lit-scoping.md`) was scoped to whether a DV/Fock-space *IQP-specific* construction exists, not to whether this particular polarization encoding's specifics have been published before; no claim of priority over other unpublished or published polarization-encoding IQP mappings is made or checked here. What is established is that the general question ("can IQP be realized in linear optics at all") was answered by KLM in 2001, and this mapping's checked correctness does not change that. See `docs/iqp-lit-scoping.md`'s 2026-09-03 addendum for the fuller citation. The sentence below is kept as the original, uncorrected framing, per this project's additive-correction convention — read it against this note, not standalone.
...
**3. Weight-≥2 diagonal generator.** Owner initially proposed CNOT/CZ/SWAP as "how multi-qubit circuits are built" generically. Corrected: IQP's middle layer is *defined* to be diagonal in the Z basis — that's the acronym's premise, and what makes the circuit commute pairwise despite being classically hard to sample (Bremner-Jozsa-Shepherd; Douce et al.'s CV analogue). Of the three gates named:
   - **CNOT** — not itself Z-diagonal. It may appear inside a decomposition whose complete logical operation is Z-diagonal; the IQP restriction applies to that complete operation.
...
`build_state_prep_circuit(n)` in `merlin_iqp/encoding/iqp_photonic.py` implements this: `HWP(π/8)` at port `2k` for each qubit `k` (even ports carry polarization, odd ports are vacuum-partner modes, untouched here).
...
Two qubits, weight-1 generators only: `θ₀ = 0.3`, `θ₁ = 1.1`, both qubits starting in `|H⟩ = |0⟩`. `merlin_iqp/encoding/iqp_photonic.py`'s `build_full_circuit(2, [0.3, 1.1])` builds the 4-mode circuit (`HWP(π/8)` ×2 → `WP(θ_k,0)` ×2 → `HWP(π/8)` ×2 → `PBS` ×2); running it through `Processor("SLOS", ...)` + `Analyzer` gives:
...
These match to floating-point precision (`tests/encoding/test_iqp_photonic_encoding.py`'s parametrized product-distribution test, which also checks `n=3` with three independent generators). Two things this concretely confirms about the *joint* two- and three-qubit state, beyond the marginal per-qubit phase-to-probability relationship already established:
...
*Correction (Plan 09-02):* an earlier version of this document and `merlin_iqp/encoding/iqp_photonic.py` had this port↔polarization assignment backwards — self-consistent within its own labels (so no numerical test result was ever wrong), but the "H"/"V" tags didn't match true physical polarization. This was caught during the ENC-03 attempt-first checkpoint when the owner asked which output pair was really H, prompting the direct bare-`PBS` check above. Fixed in both the module and its tests (`tests/encoding/test_iqp_photonic_encoding.py`'s parametrized closed-form test now reads `H` from `BasicState([0,1])` and `V` from `BasicState([1,0])`); all 12 tests still pass, since the fix is a consistent relabeling, not a change to the underlying physics.
...
ENC-01 is implemented and tested: `merlin_iqp/encoding/iqp_photonic.py` (state-prep/diagonal-layer/conjugation/readout circuit builders, weight-1 generators) and `tests/encoding/test_iqp_photonic_encoding.py` (12 tests, all passing) — `python -c "import merlin_iqp.encoding.iqp_photonic"` and `pytest tests/encoding/test_iqp_photonic_encoding.py -v` both verified clean.
...
**Confirmed correct, and it caught a real bug.** `merlin_iqp/encoding/iqp_photonic.py`'s Wave 1 `basic_state_to_bitstring` helper had this backwards (`(1,0)→"H"`, `(0,1)→"V"`) — self-consistent within its own labels (no Wave 1 test result was ever numerically wrong), but the labels didn't match true physical polarization. Fixed in this plan across the module, its tests, and ENC-01's derivation text (see the "Correction" note under Ingredient 1, and the commit history) — all 12 pre-existing tests still pass after the relabeling, since it's a consistent rename, not a physics change.
...
**3. Falsifiability.** Guided question: what should you get back if you forward-map a bitstring then immediately reverse-map it? Owner's answer: *"You should get the same bitstring. Quantum operations should be unitary, meaning they are reversible..."* — correct mechanism and correct expected behavior; this is exactly `tests/encoding/test_iqp_photonic_encoding.py`'s `test_enc03_round_trip`, and it's the same style of check that caught the H/V bug in point 1.
...
- `(2,0)`/`(0,2)` — both coins bunched into the *same* box (the Hong-Ou-Mandel-style bunching phenomenon from `scripts/v1_generator/perceval_fluency_demo.py`'s Phase 8 demo, applied to two photons that should have gone to different modes).
...
Verified by `pytest tests/encoding/test_iqp_photonic_encoding.py -v` — 24/24 passed, including 7 round-trip cases (`n=1,2,3`) and 5 out-of-subspace cases (all four invalid patterns individually, plus one embedded in a larger valid register).
...
The owner also asked about using MMD loss instead of total variation distance. The original rationale incorrectly treated kernels as necessary only for sampling noise. MMD is meaningful for exact probability vectors too. TVD is used here as a bandwidth-free discrepancy between normalized finite distributions; this choice does not make exact-distribution MMD invalid. See [MMD conventions](mmd-loss.md).
...
**What this extends:** since `heralded_cz` is a fixed catalog gate, Ingredient 2's `heralded_cz`-based derivation above realizes `exp(iθZ_iZ_j)` only at the fixed angle `θ=π/4`. `PostProcessedControlledRotationsItem` (a **different** gate family — post-selection on ancilla vacuum + per-qubit data validity, not `heralded_cz`'s ancilla heralding) implements a continuously-tunable `CP(α) = diag(1,1,1,e^{iα})`, de-risked standalone in Phase 15 (`scripts/v3_arb_gate/cp_gate_derisking.py`, `tests/scripts/v3_arb_gate/test_cp_gate_derisking.py`, 8/8 passing) and confirmed to match `heralded_cz`'s boundary exactly at `α=π` (not `α=π/4` — see the correction below). This section derives the general-`α` operator identity connecting it to `exp(iθZ_iZ_j)` for arbitrary `θ`, and the gate's success probability as a closed-form function of `α`.
...
**Verification** — `p_success(α)` computed from the closed form above vs. `scripts/v3_arb_gate/cp_gate_derisking.py`'s independently-measured `|amplitude|²` table (all 4 computational-basis inputs give the same value at each `α`, matching Success Criterion 2's confirmed uniformity):

| α | closed-form `p_success(α)` | measured (`scripts/v3_arb_gate/cp_gate_derisking.py`) | diff |
|---|---|---|---|
...
**Measured results** (`tests/encoding/test_iqp_photonic_encoding.py`, all TVD figures at floating-point-noise level, well under the locked `<1e-6` bar — the same bar `heralded_cz`'s fixed-angle construction cleared, not relaxed for this different mechanism):
...
`scripts/v3_arb_gate/cp_alpha_sweep.py` (repo root) runs the sweep and produces:

- `results/v3_arb_gate/phase16_alpha_sweep.csv` — raw data: `alpha, measured_success_prob, closed_form_success_prob` for all 16 points.
- `results/v3_arb_gate/phase16_alpha_sweep.png` — the closed-form curve plotted continuously (200 densely-sampled points), with the 16 measured points overlaid as scatter markers:

![CP(alpha) success probability sweep](../results/v3_arb_gate/phase16_alpha_sweep.png)
...
See `forge/ancilla_mapping.frg` (the model) and `results/v3_arb_gate/phase16_forge_summary.md` (the pass/fail record and raw `racket` output).
...
Full derivation, the vertex-sharing argument, and the even-`n` construction proof: `results/v3_forge_formal/phase22_allocation_invariant.md`.

### No Python implements this — the direction of truth is inverted

**No Python implements this scheme.** No code in `merlin_iqp/encoding/iqp_photonic.py` implements a k-pair pooled circuit. This scheme is a specification, not a description of shipped behaviour. `forge/pooled_ancilla_allocation.frg`, together with `results/v3_forge_formal/phase22_allocation_invariant.md`'s prose invariant, is therefore the **source of truth** any future implementation must be checked against — the model was written and verified before any Python exists to drift from it.
...
Full timing tables, per-`n` breakdowns, and the verbatim Forge output are in `results/v3_forge_formal/phase22_forge_summary.md` and `results/v3_forge_formal/phase22_forge_run_log.md`.
...
Verbatim from `results/v3_forge_formal/phase22_forge_summary.md`: **"A few hundred lines of backtracking Python reached the same minimum faster, and reached further, than Forge's SAT-backed exhaustive search."**
...
The live n=4 run used the six K4 pairs, two gates, one four-mode block, nine ordered states, and `for 7 Int`. The exact command, bound, solver output, and timings are preserved in [`results/v3_forge_formal/phase23_lifecycle_run_log.md`](../results/v3_forge_formal/phase23_lifecycle_run_log.md); the readable state-by-state projections are in [`results/v3_forge_formal/phase23_lifecycle_traces.md`](../results/v3_forge_formal/phase23_lifecycle_traces.md).
...
The full evidence summary, including the owner-reviewed interpretation, is [`results/v3_forge_formal/phase23_lifecycle_summary.md`](../results/v3_forge_formal/phase23_lifecycle_summary.md).

### Phase 22 cross-check and static minimum-K boundary

Phase 22's MPAIR-07 Perceval probe measured pooled-versus-dedicated output distributions for the n=4 vertex-disjoint configuration and recorded `tvd_pooled_vs_dedicated` values of approximately `1.305e-14` and `2.899e-14`, inside the pre-committed `1e-9` tolerance (`results/v3_forge_formal/phase22_reuse_gate.md`). Phase 23 measures structural lifecycle liveness under the same strict deferred-post-selection interpretation. The same-trace numerical GO and structural unsafe witness are retained as an unresolved abstraction-level disagreement. LIFE-05 does not try to prove one method wrong. Cross-epoch reuse is a separate safe-witness sanity check.
