...
`_build_weight2_cp_processor_no_postselect`'s local→global ancilla mode-mapping dict (`iqp_photonic_encoding.py:622-627` — `{2i:0, 2i+1:1, 2j:2, 2j+1:3, 2n:4, 2n+1:5, 2n+2:6, 2n+3:7}`) is the real analog of what a literal `Processor.set_postselection()` local→global translation would otherwise do; no such literal call exists in the shipped pipeline, since `set_postselection()` raises `AssertionError` when composed with this pipeline's later components (Phase 15's Pitfall 3) — filtering is done by hand instead.

Phase 16 formally checks that this dict's 8 keys are pairwise distinct (injective) for every valid `(n, i, j)`, `0 ≤ i,j < n`, `i ≠ j`, checked against **all** `n` qubits' own data ports (`0..2n-1`) — not just qubit `i`/`j`'s — using [Forge](https://forge-fm.org/) (a Racket-hosted relational model finder), rather than asserting the property informally. Two checks: a `sat` non-vacuity check (a valid instance exists) and an `unsat` non-collision check (no counterexample within `n ≤ 8`). Both passed — the mapping is confirmed injective and non-aliasing, no bug found. See `forge/ancilla_mapping.frg` (the model) and `results/phase16_forge_summary.md` (the pass/fail record and raw `racket` output).
