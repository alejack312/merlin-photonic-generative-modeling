...
ENC-01 is implemented and tested: `merlin_iqp/encoding/iqp_photonic.py` (state-prep/diagonal-layer/conjugation/readout circuit builders, weight-1 generators) and `tests/test_iqp_photonic_encoding.py` (12 tests, all passing) — `python -c "import merlin_iqp.encoding.iqp_photonic"` and `pytest tests/test_iqp_photonic_encoding.py -v` both verified clean.
...
- `(2,0)`/`(0,2)` — both coins bunched into the *same* box (the Hong-Ou-Mandel-style bunching phenomenon from `scripts/perceval_fluency_demo.py`'s Phase 8 demo, applied to two photons that should have gone to different modes).
...
**Verification** — `p_success(α)` computed from the closed form above vs. `scripts/cp_gate_derisking.py`'s independently-measured `|amplitude|²` table (all 4 computational-basis inputs give the same value at each `α`, matching Success Criterion 2's confirmed uniformity):

| α | closed-form `p_success(α)` | measured (`scripts/cp_gate_derisking.py`) | diff |
|---|---|---|---|
