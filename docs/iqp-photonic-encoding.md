...
- **Generator-weight scope.** Weight-1 generators are fully derived, implemented, and empirically validated (ENC-01, ENC-04). Weight-2 generators are now also implemented and validated (Phases 11-13, v2.1): `build_cz_insertion`/`build_weight2_processor` realize the circuit, Phase 12's TVD test confirms it matches the extended exact reference (TVD=2.58e-15 at n=2, θ=π/4), and Phase 13 confirms weight-1/weight-2 composability at n=3. The remaining limitation is scope, not validation: weight-2 realizes a single fixed angle (`π/4`) only, not an arbitrary θ — resolving that (ARB-01) is deferred to a future milestone.
