...
**Phases 14-21 + 17.1 are complete — all 8 shipped and verified, all 37 requirements defined at that point satisfied. Phase 22 was added 2026-08-20 as additive v3.0 scope, so the milestone is no longer closed.** Phase 21's plan 21-01 also still carries an open `checkpoint:human-verify` loose end outside this repo: the `alejandro-jackson` case-study page is approved and committed locally but **not pushed** — a deliberate, separate owner action, tracked here so it is not lost.
