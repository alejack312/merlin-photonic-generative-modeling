...
Plans:
- [x] 13-01-PLAN.md — n=3 mixed weight-1 + weight-2 composability test (WT2-07)

**Results (measured 2026-08-06):** `test_wt2_composability_mixed_generators_n3` added to `tests/test_iqp_photonic_encoding.py` — 3 parametrized configs at n=3, each stacking a nonzero weight-1 theta on a weight-2 pair member (i or j) plus one on the bystander qubit outside the pair, alongside a weight-2 (ZZ) term. Primary check: TVD < 1e-6 against the extended exact reference for all 3 configs (independently re-verified). Companion sanity check: TVD against the weight-1-only exact reference is 0.46-0.50 (>> the 0.1 threshold), confirming the ZZ term has a genuine non-vacuous effect rather than composing to a no-op. Full suite: 118/118 passing (115 pre-existing + 3 new), confirmed via a clean before/after diff of exactly +3. No production code changed — `exact_qubit_iqp_distribution` and `photonic_weight2_iqp_distribution` already supported this scenario from Phase 12. Verified by gsd-verifier: 3/3 must-haves, independently re-executed. All 8/8 v2.1 requirements (WT2-01 through WT2-08) now Complete — v2.1 Weight-2 Implementation milestone functionally done, pending owner interpretation and formal milestone close.
