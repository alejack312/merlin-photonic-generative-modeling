...
- [x] **WT2-02**: `exact_qubit_iqp_distribution` (or a sibling function) extended to accept weight-2 pair terms (`Z_iZ_j`), reusing the existing bit-ordering convention

### Validation

- [x] **WT2-03**: Herald-conditioned photonic distribution computed, with herald-failure probability and out-of-subspace decode residual reported as two separate, explicit numbers — never merged into one figure, never silently renormalized away, per this project's established ENC-03 honesty-ledger policy
...
- [x] **WT2-05**: TVD validation test at n=2, θ=π/4, comparing the extended exact qubit-side reference (WT2-02) against the herald-conditioned photonic distribution (WT2-03), in the same style/tolerance conventions as the existing `test_enc04_toy_validation_runs_end_to_end`
- [x] **WT2-06**: Test coverage added to `tests/test_iqp_photonic_encoding.py` for WT2-01 through WT2-05, matching existing test conventions
- [x] **WT2-07**: n=3 mixed generator test — 2 weight-1 terms plus 1 weight-2 term in the same circuit, confirming weight-1 and weight-2 layers compose correctly
...
| WT2-02 | Phase 12 (Exact Reference Extension & TVD Validation) | Complete |
| WT2-03 | Phase 12 (Exact Reference Extension & TVD Validation) | Complete |
| WT2-04 | Phase 10 (Heralded-CZ Primitive De-Risking) | Complete |
| WT2-05 | Phase 12 (Exact Reference Extension & TVD Validation) | Complete |
| WT2-06 | Phase 12 (Exact Reference Extension & TVD Validation) | Complete |
| WT2-07 | Phase 13 (Weight-1 + Weight-2 Composability Validation) | Complete |
