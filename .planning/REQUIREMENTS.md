...
- [x] **MPAIR-06**: The verified scheme recorded as a **specification for future implementation** in `docs/iqp-photonic-encoding.md`, explicitly stating that no Python implements it yet — so unlike `forge/ancilla_mapping.frg` (which re-states an already-shipped formula with nothing linking the two, and carries a standing drift warning because of it), this model is the source of truth any eventual implementation must be checked against.
...
| MPAIR-03 | 22 - Multi-Pair Ancilla Allocation (Forge) | Complete |
| MPAIR-04 | 22 - Multi-Pair Ancilla Allocation (Forge) | Complete |
