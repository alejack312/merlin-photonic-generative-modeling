...
Phase: 12 of 13 (Exact Reference Extension & TVD Validation) — ready to discuss/plan.
Plan: — (not yet planned)
Status: Phase 11 completed and verified 2026-08-06 (gsd-verifier: 4/4 must-haves passed, no gaps, independently re-executed rather than trusted from SUMMARYs). `build_weight2_processor(n, i, j, thetas)` assembles the full weight-2 IQP generator pipeline as a `Processor(2n+2)` via `Processor.add()`: state prep → π/4-folded diagonal layer → `build_cz_insertion` (Plan 11-01) wired via an explicit mode-mapping dict → conjugation → readout, reusing every weight-1 builder unmodified. Heralds registered immediately after the CZ insertion's `.add()` call, confirmed at exact global indices `{2n: 1, 2n+1: 1}`. Discovered and worked around a genuine Perceval limitation mid-execution: `Processor.add_herald()` combined with any `PBS`-containing circuit crashes `Processor.probs()` unconditionally (`matmul` shape mismatch in `PolarizationSimulator._prepare_input`), and separately, real Hadamard-created superposition (`build_state_prep_circuit`) feeding a heralded-ancilla sub-circuit gives silently wrong (non-crashing) probabilities — both confirmed by direct execution (independently reproduced twice more: once by the orchestrator, once by gsd-verifier) and flagged as a **blocking concern for Plan 12** (see Blockers/Concerns below). WT2-01 satisfied — marked Complete in REQUIREMENTS.md. Manual staff-engineer review (gstack's `/review` skill couldn't run — this repo works directly on `master` with no feature branch/PR, and the skill requires a base-branch diff): no code issues found; one documentation-staleness item found and fixed (`docs/iqp-photonic-encoding.md` still said weight-2 was "derived on paper but not built/tested," now corrected to point at the implementation). Full repo suite: 107/107 tests pass, no regressions to the pre-existing 32-test weight-1 suite. Build order: Phase 10 (done) → Phase 11 (done) → Phase 12 (exact reference extension + TVD validation, next) → Phase 13 (weight-1/weight-2 composability).

Progress: v1.0 [██████████] 100% (GEN-07 honestly concluded not-met within that 100% — not a partial-completion asterisk, the requirement was fully addressed, the outcome was negative). Phase 7 mechanism-validation work is fully closed, owner interpretation pending. v2.0 [██████████] 100% — both phases complete (Phase 8: 4/4 plans; Phase 9: 4/4 plans), 11/11 requirements satisfied, shipped 2026-08-05. v2.1 [█████░░░░░] ~50% — Phase 10 complete and verified (1/4 phases), Phase 11 complete (2/4 phases, 2/2 plans).
...
- **Confirmed Perceval limitation (blocking concern for Plan 12):** `Processor.add_herald()` combined with any `PBS`-containing circuit crashes `Processor.probs()` unconditionally (`matmul` shape mismatch inside `PolarizationSimulator._prepare_input`), independent of theta values or whether `build_state_prep_circuit`'s Hadamard is present. Separately, even without `add_herald`, routing a real Hadamard-created superposition (`build_state_prep_circuit`) into a heralded-ancilla sub-circuit gives silently wrong (non-crashing) probabilities via `PolarizationSimulator` — confirmed by direct execution, extends Phase 11 Plan 01's already-flagged ancilla-photon-distinguishability concern to real superposition inputs (not just directly-constructed dual-rail states). Plan 11-02 worked around this for its own sanity check (bare/herald-unregistered processor + manual post-selection + definite computational-basis input, skipping `state_prep`), but this is NOT a general fix — Plan 12's TVD validation, which needs `build_weight2_processor`'s actual output distribution (including superposition, since IQP generators are meaningless without it), cannot use `Processor.probs()` on the fully composed, herald-registered pipeline as-is and must plan its measurement strategy around this constraint from the start.
...
- Next: Phase 11 complete and verified — discuss/plan Phase 12 (Exact Reference Extension & TVD Validation) via `/gsd:discuss-phase 12`, which must design its measurement strategy around the confirmed Perceval `Processor.probs()` + `add_herald` + `PBS` limitation (see Blockers/Concerns below) from the start, not discover it mid-execution
...
**One open concern, flagged for Plan 12 planning:** `Processor.add_herald()` combined with any `PBS`-containing circuit crashes `Processor.probs()` unconditionally (confirmed by direct execution during Phase 11 Plan 02, `matmul` shape mismatch inside Perceval's `PolarizationSimulator._prepare_input`, independent of thetas or state_prep). Separately, real Hadamard-created superposition feeding a heralded-ancilla sub-circuit gives silently wrong (non-crashing) probabilities via the same simulator. Plan 12's TVD validation needs `build_weight2_processor`'s actual output distribution (which requires both heralds AND superposition) and cannot use `Processor.probs()` on the fully composed pipeline as-is — see `.planning/phases/11-cz-insertion-unit-weight-2-circuit-composition/11-02-SUMMARY.md`'s "Issues Encountered" and "Next Phase Readiness" for the full writeup and a recommended starting point for a workaround. This needs to be resolved (or a viable measurement strategy found) as part of Phase 12 planning, not deferred silently.
...
Stopped at: Phase 11 executed (2 plans), verified (gsd-verifier: 4/4 must-haves, no gaps), manually code-reviewed (one doc-staleness fix applied), and closed out — `.planning/ROADMAP.md`, `.planning/REQUIREMENTS.md` (WT2-01 → Complete), `.planning/phases/11-cz-insertion-unit-weight-2-circuit-composition/11-VERIFICATION.md`, and `.planning/STATE.md` all updated and committed.
Resume by: discuss/plan Phase 12 (Exact Reference Extension & TVD Validation) via `/gsd:discuss-phase 12` — read this session's flagged Perceval `Processor.probs()`/`add_herald`/`PBS` limitation first (STATE.md Blockers/Concerns, 11-02-SUMMARY.md) and design the TVD measurement approach around it from the start.
