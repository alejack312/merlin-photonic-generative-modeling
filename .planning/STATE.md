...
Phase: 11 of 13 (CZ Insertion Unit & Weight-2 Circuit Composition) — Plan 02 of 2 complete. Phase 11 complete.
Plan: 02 complete — `build_weight2_processor(n, i, j, thetas)` implemented; all 4 of Phase 11's ROADMAP Success Criteria satisfied.
Status: Phase 11 Plan 02 executed 2026-08-06. `build_weight2_processor(n, i, j, thetas)` assembles the full weight-2 IQP generator pipeline as a `Processor(2n+2)` via `Processor.add()`: state prep → π/4-folded diagonal layer → `build_cz_insertion` (Plan 11-01) wired via an explicit mode-mapping dict → conjugation → readout, reusing every weight-1 builder unmodified. Heralds registered immediately after the CZ insertion's `.add()` call, confirmed at exact global indices `{2n: 1, 2n+1: 1}`. Discovered and worked around a genuine Perceval limitation mid-execution: `Processor.add_herald()` combined with any `PBS`-containing circuit crashes `Processor.probs()` unconditionally (`matmul` shape mismatch in `PolarizationSimulator._prepare_input`), and separately, real Hadamard-created superposition (`build_state_prep_circuit`) feeding a heralded-ancilla sub-circuit gives silently wrong (non-crashing) probabilities — both confirmed by direct execution and flagged as a **blocking concern for Plan 12** (see Blockers/Concerns below). Full repo suite: 107/107 tests pass, no regressions to the pre-existing 32-test weight-1 suite. Phase 11 is now complete — Build order: Phase 10 (done) → Phase 11 (done) → Phase 12 (exact reference extension + TVD validation, next) → Phase 13 (weight-1/weight-2 composability).
...
- Next: Phase 11 complete (both plans done) — plan Phase 12 (Exact Reference Extension & TVD Validation), which must design its measurement strategy around the confirmed Perceval `Processor.probs()` + `add_herald` + `PBS` limitation (see Blockers/Concerns below) from the start, not discover it mid-execution
...
Stopped at: Phase 11 Plan 02 executed — `build_weight2_processor(n, i, j, thetas)` added to `iqp_photonic_encoding.py`; herald/sanity/regression tests added to `tests/test_iqp_photonic_encoding.py`; `.planning/phases/11-cz-insertion-unit-weight-2-circuit-composition/11-02-SUMMARY.md` created; `.planning/STATE.md` refreshed. Phase 11 is now complete (2/2 plans, all 4 ROADMAP Success Criteria satisfied).
Resume by: plan Phase 12 (Exact Reference Extension & TVD Validation) via `/gsd:plan-phase 12` — read this session's flagged Perceval `Processor.probs()`/`add_herald`/`PBS` limitation first (STATE.md Blockers/Concerns, 11-02-SUMMARY.md) and design the TVD measurement approach around it from the start.
