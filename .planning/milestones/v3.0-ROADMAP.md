...
**MerLin dual-rail addendum (2026-08-18):** the complete Phase 18 grid was rerun through the polarization-free MerLin `QuantumLayer` parallels (weight-1 n=2..6, mixed n=2..4, five matched draws). MerLin's loss transform agrees with explicit LC on identical dual-rail circuits at float32 precision, and the full study reproduces loss-induced TVD and herald-compounding behavior while retaining real encoding-dependent differences in lossless output shape. Separate datasets, plots, and a 56-cell backend comparison are linked from `docs/hardness-under-loss-study.md`.
...
**Result: Complete — verified 5/5 must-haves.** All four independent cross-checks reached a genuine GO: VERIFY-02 (Yao.jl qubit-side, TVD 2.26e-17/1.12e-16 at n=2/3), VERIFY-03 weight-1 (BosonSampling.jl, TVD 2.36e-16/3.04e-16 at n=2/3), VERIFY-03 weight-2 (Knill-CZ, TVD 3.50e-15 — a real matrix-transpose convention bug found via Knill's own Eq. 6 zero-leak constraint, fixed, and independently confirmed via Codex/gpt-5.5 review to be a legitimate correction, not a coincidental double-bug), and VERIFY-04 (BosonSampling.jl native loss API, TVD ≤1.75e-14 across 3 η values for both weight-1 and mixed scope — two real findings: η-as-transmission-amplitude-not-probability, and a package dispatch gap, both fixed and documented). `docs/julia-cross-check-study.md` written as the canonical results document; `REQUIREMENTS.md`'s VERIFY-02/03/04 rows corrected to Complete. gsd-verifier independently re-ran all four Julia scripts live rather than trusting SUMMARY claims. A follow-up Codex review (requested by the owner) confirmed the transpose fix's reasoning holds up, but flagged a real, separate methodological gap: the weight-2 locked-gate test case (thetas=[0,0]) produces a distribution symmetric under bit-relabeling (00/11 at 50% each, 01/10 both exactly 0), which couldn't fully rule out a hidden Julia/Python bitstring-convention mismatch. **Follow-up (same day):** a second asymmetric-theta case (thetas=[0.3, 1.1], `weight2_asymmetric_n2.csv`) was added and passes (TVD=4.50e-15), which does catch a single-qubit rail-convention error. The residual (a simultaneous both-qubits bit-flip or qubit-order-swap mislabeling remaining undetectable) was then run down to an actual proof, checked against the primary source directly (Hein, Eisert & Briegel, Phys. Rev. A 69, 062311 (2004), arXiv:quant-ph/0307130): this circuit's diagonal layer realizes the two-vertex `CZ|++⟩` graph state up to local phase, whose Eq.-41 single-qubit reduced state is exactly maximally mixed for any theta, forcing `P(00)=P(11)`/`P(01)=P(10)` exactly via the two uniform marginals plus normalization — matching the closed form `(1±cos2θᵢcos2θⱼ)/4` to machine precision. Specific to the `π/4`-locked pair angle (the only one `heralded_cz` realizes), not a generic IQP property. Full derivation in `results/phase19_verify03_weight2_results.md`; owner reviewed and chose to ship this improvement rather than open further scope (e.g. n=3).
...
**Result: Complete — verified 6/6 must-haves.** TRAIN-07's genuinely-open self-explanation gap was closed with the owner's own transcribed reasoning (Plan 20-01); all three source docs (`docs/trainability-study.md`, `docs/hardness-under-loss-study.md`, `docs/iqp-photonic-encoding.md`) gained their own WRITE-02 literature comparison tables and, for TRAIN/HARD, a Herbst et al. cross-reference note satisfying success criterion 6 (finding: HARD's measured `alpha(eta)` direction is the *opposite* of `docs/iqp-baseline.md`'s original 2026-08-12 speculative guess — loss increases anticoncentration here, it doesn't erode it — explicitly correcting that earlier note); `docs/technical-findings.md` (Plan 20-04) mirrors all three tables and points at (without re-deriving) each source doc's scope statement, the Herbst cross-thread, and `docs/julia-cross-check-study.md`'s supplementary independent verification. gsd-verifier independently confirmed all 6 success criteria against actual file content, not SUMMARY claims — see `20-VERIFICATION.md`. A scoped staff-engineer-style review (commits `2053610..2f5a581`, since gstack's `/review` skill assumes a PR workflow this repo doesn't use) found citation accuracy, internal link/anchor resolution, and traceability of every quoted number all correct; one pre-existing (Phase-18-introduced, not Phase-20) malformed-heading defect was noted as informational only. WRITE-01 through WRITE-06 all satisfied.
...
Plans:
- [x] 21-01-PLAN.md — Extend the alejandro-jackson case-study page with v3.0 TRAIN/HARD co-lead findings + ARB-01/Julia supporting evidence + updated Role/authorship section
- [x] 21-02-PLAN.md — Add a short v3.0 pitch section to this repo's README, linking out to the case-study page and docs/technical-findings.md

**Result: Complete — verified 5/5 must-haves.** Plan 21-01's checkpoint went through four owner-review rounds before approval, each substantive: round 1 found 6 real content gaps (missing homepage feature, stale hero description, missing Julia/Forge badges, no v2.0/2.1/3.0 "What I'd Do Next" items, no v2.0→v3.0 TL;DR, an unexplained R²=0.000 cell); round 2 found a real accuracy gap (the page implied MerLin `QuantumLayer` autograd was never used, omitting a supplementary dual-rail reimplementation that does run through it); round 3 added an owner-requested "Bigger Picture" section comparing this project against the sibling `iqp-mmd-barren-plateau` project, closing with the owner's own transcribed verdict ("Not promising, on my own read of it... still worth exploring further") — during which a drafting error of Claude's own was caught against source data and corrected before shipping. A post-approval staff-engineer-style review (`fullstack-dev` agent, since gstack's `/review` skill assumes a PR workflow neither this repo nor `alejandro-jackson` uses) found one real dangling reference (a homepage card's `R²=0.999*` asterisk with no footnote anywhere on the page) — fixed directly, along with aligning that card's outcome text to match the owner's own already-made hero-subtitle wording softening ("v3.0 study"/"didn't exist before" → "broader study"/"not widely explored"). Four low-severity reuse/consistency findings (duplicated JSX block in "What I'd Do Next," a few `CalloutBox`-reuse opportunities, one pointless per-render `.map()`, header-casing inconsistency with zero visual effect since `DataTable` already uppercases via CSS) were reported, not fixed — informational only, not blocking. `alejandro-jackson`'s `main` ended 5 content commits + 1 fix commit ahead of `origin/main` at the time this was written. **Since corrected (2026-08-20): those commits are pushed and the repo is at 0/0 with `origin/main`.** Six further commits landed after the round-3 commit `28ad9dc` — `4fe7b71` (dangling asterisk / outcome wording), `4367dd3` (TVD-metric-artifact insight), `4c5d9f8` (anticoncentration claim correction), `1ef9c71` (ZZ-hardness scope precondition stated up front), `631101c` (backend output-shape difference explained precisely), `758a1a5` (homepage MerLin card metrics relabelled to read in the right direction), and `14299cd` (honest note on the Forge check's actual contribution, cross-repo sibling of this repo's `273e9dd`). Two of those are substantive accuracy fixes, not polish — worth noting that owner review kept finding real defects past the formal approval round. See `.planning/phases/21-external-facing-framing-pass/21-VERIFICATION.md` and `21-01-SUMMARY.md`'s four checkpoint-continuation rounds.
...
**Goal:** Symbolically verify a k-pair ancilla allocation scheme for multi-ZZ weight-2 IQP circuits **before** any of it is implemented — the one place in this project where formal methods pay their way, since the property must hold across every subset of pairs (at n=8 there are C(8,2)=28 candidate pairs, so 2^28 ≈ 268M subsets), a violation would fail silently as a mode collision, and the counterexample would be a *structure* rather than a number. Deliberately scoped to verification only: no Python k-pair implementation, no multi-ZZ hardness re-run.
...
**Attempt-first gate (MPAIR-01):** The allocation scheme is a conceptual design decision, so per CLAUDE.md it is the owner's to make. Claude presents 2–3 candidates — contiguous per-pair blocks, interleaved, and a pooled/recycled-ancilla variant — with tradeoffs stated and **no implied ranking**, and the owner picks. Note that the candidates differ in how hard the property actually is: a strictly contiguous `2n+4m … 2n+4m+3` scheme is close to true-by-construction (the same weakness ARB-09's audit found), whereas a pooled scheme that recycles ancillas across non-overlapping pairs is genuinely combinatorial. That difference is part of what the owner is choosing between, and it should be stated plainly at the gate rather than discovered afterward.

**Success criteria:**
1. The owner has selected one allocation scheme from ≥2 presented candidates, with the rejected candidate(s) and the reason recorded in writing — the selection is demonstrably the owner's, not Claude's.
2. A Forge model of the selected scheme runs clean: the non-collision test returns `unsat` (no counterexample within bound) **and** the non-vacuity test returns `sat`.
3. The `n`/`k` bound and the `Int` bitwidth are each justified in-model against the largest value the model computes, so no silent overflow is possible — matching `forge/ancilla_mapping.frg`'s existing bitwidth note.
4. A brute-force baseline over the same bounded domain has been run and timed, and the phase states plainly whether Forge's exhaustive search earned its place here or — as ARB-09's audit concluded at the single-pair scale — did not. Either verdict satisfies the criterion; only an unchecked assertion fails it. *(2026-08-20: under D-05's search framing the baseline is a colouring search, not a verification loop.)*
...
- [x] 23-01-PLAN.md — Explicit relational State/next lifecycle model, strict deferred-postselection transitions, and safe/unsafe witness predicates (LIFE-01..04)
- [x] 23-02-PLAN.md — Bounded Forge execution, raw run log, readable traces, and Phase 22 numerical cross-check record (LIFE-02..05)
- [x] 23-03-PLAN.md — Static-vs-temporal synthesis, owner interpretation checkpoint, canonical documentation, and requirement closeout (LIFE-05..07)
