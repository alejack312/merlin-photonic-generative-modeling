...
| F02 NAT control | PASS | Matched continuation carries pair keys, windings and Adam state; direct-resume trajectory identity is tested and an n=4 artifact exists. The fixed-pair run is labeled ablation. | Add the full NAT target-improvement/fixed-reference/acceptance report required by NAT-02. | None for ideal; D1 only for a noisy arm |
...
| SWEEP-03 | INCONCLUSIVE | True KL now exports JSON-safely with explicit infinity status, zero-shot coverage is corrected, and core metric tests pass; the marginal/source-AC panel, occupancy artifact and independent hand fixtures are still incomplete. | Implement and artifact-link the remaining metrics without fabricating exact vectors from moments. | D1 only for a noisy arm |
...
| NAT-02 | INCONCLUSIVE | Fifteen registered 150-step NAT artifacts, a fixed-pair ablation, and n=4 matched-arm artifacts branching from one frozen warm start with identical optimizer state now exist. The branch contract is verified, but the required report fields for target improvement, fixed-reference distance and acceptance are not yet artifacted for the full selected comparison. | Add the required NAT metric panel and run the registered matched profile comparison within the existing budget; keep the fixed-pair procedure labeled as an ablation. | None for ideal continuation; D1 only if a noisy arm is selected |
...
| REPRO-03 | INCONCLUSIVE | The recorded synthetic `training_smoke` data was regenerated from the sibling recipe/seed and the source SGD trajectory was independently rerun into an isolated output tree with exact theta/loss agreement. Bandwidth/Ghosh–Kim replay and retraining rows remain unexecuted; checkpoint replay remains separately labeled adapted reproduction. | Run the registered bandwidth/Ghosh–Kim replay and retraining cells where their source data/checkpoints are present; disposition genuinely external inputs separately. | None for training_smoke; external source inputs for missing rows |
