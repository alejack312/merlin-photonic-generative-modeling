...
| NAT-02 | INCONCLUSIVE | Fifteen registered 150-step NAT artifacts, a fixed-pair ablation, and an n=4 matched-continuation warm-start/optimizer-state artifact now exist. The continuation trajectory identity is unit-tested, but the required report fields for target improvement, fixed-reference distance and acceptance are not yet artifacted for the full selected comparison. | Add the required NAT metric panel and run the registered matched profile comparison within the existing budget; keep the fixed-pair procedure labeled as an ablation. | None for ideal continuation; D1 only if a noisy arm is selected |
