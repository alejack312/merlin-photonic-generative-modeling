...
| SWEEP-04 | INCONCLUSIVE | Bounded derived comparison artifacts now cover Hamming n=6 seeds 0–4, Hamming n=8 seeds 0–4, and spatial n=6 seeds 0–1, with raw/compiled/deployed-map panels and metric controls. A complete sibling matched arm, full spatial replication, sampled uncertainty, and larger-n physical arm remain absent. | Add the exact sibling arm and remaining registered spatial cells only after the sibling inputs and physical boundary are available; regenerate paired figures without treating analytic deployed-map references as physical results. | External sibling inputs; larger-n physical validation; no new owner choice |
