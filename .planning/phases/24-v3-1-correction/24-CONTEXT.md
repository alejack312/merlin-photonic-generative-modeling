...
- **2026-09-03, NULL-02 tolerance widened (rel 0.35 → 0.5) for the TRAIN ratio test — SUPERSEDED same day, see next entry.** ~~3 of 129 rows (weight1 n=6, the largest swept n; mixed n=3,4, the smallest) came in 35-38% off a Monte-Carlo closed-form estimate. Confirmed via a 5-seed stability check this is a real, stable, small model discrepancy at the edges of the swept range, not sampling noise in the null-model's own estimate — accepted as a finite-size effect on a scaling-law claim rather than chased further, given the ship-first priority above.~~ This diagnosis was wrong (see below); kept struck through rather than deleted per this project's additive-correction convention.
- **2026-09-03, TRAIN null model bug found by a parallel Fable 5.1 session, independently reverified:** the "finite-size effect" above was actually a bug — `owner_train_null_ratio`'s helper only ever differentiated `theta[0]`, while the real sweep (`sweep.py`'s `pooled_gradients_for_cell`) pools gradients over `pick_tracked_indices(n, 3)`, up to 3 evenly-spaced parameter indices. Independently reproduced before trusting the claim: pooling over the correct indices drops every row's error from 35-47% to 0.5-10.0% (2000-draw Monte Carlo vs. the shipped 100-draw sweep). Fixed in `test_null_results.py`; tolerance correspondingly tightened `rel=0.5 → rel=0.2`. Full 129-row suite reconfirmed green 2026-09-03 with the fix.
- **2026-09-03, hardness doc's throughput headline scoped to `heralded_cz`, found by the same Fable session, independently reverified:** `docs/hardness-under-loss-study.md`'s `1/(eta^(n+2k)·(2/27)^k)` throughput cost and "one gate costs more than six qubits" line were stated as if general to any two-qubit gate in this project. Confirmed false by reading `build_cp_insertion` (`iqp_photonic.py:295-367`): the project's other two-qubit gate, tunable `CP(alpha)` (ARB-01), has all-vacuum ancilla — nothing for `pcvl.LC(1-eta)` to remove there — so its loss cost is `eta^n` regardless of gate count, with a separate loss-independent `1/sigma_max(alpha)^4` post-selection cost per gate, structurally cheaper than `heralded_cz` under loss. No loss sweep was ever run against `CP(alpha)` in this project, so only the qualitative/structural fact is stated, not a new throughput table. Fixed in `docs/hardness-under-loss-study.md`, mirrored in `docs/technical-findings.md`, `README.md`, and the public case-study page.
- COMM-01 send/hold: **sent** (owner sent the correction note to Vincent directly, 2026-09-04). Draft and send happened outside this session/repo — per D-03, Claude's role was to check the draft against the corrected docs for accuracy, but the draft was never routed through Claude for that check before sending. Recorded as the owner reported it; the accuracy check named in D-03 did not happen.
