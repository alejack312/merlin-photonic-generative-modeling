...
- **STRETCH attempt (n=7 weight-1, n=6 mixed) was launched, then manually stopped by the owner shortly after this plan's completion, before reaching n=6 mixed.** Finding: weight-1 n=7 is unreachable on this hardware -- all 4 attempted 20-draw chunks failed identically, each hitting `MemoryError: bad allocation` on their very first circuit evaluation (not a cross-call leak like the n=5/n=6 cases the CORE-sweep fixes above addressed; a single n=7 evaluation genuinely does not fit in available memory here). No stretch CSV or `.npy` chunk file was ever produced; the owner chose to stop rather than let the job keep failing for its remaining ~30 projected hours. This is a real, useful finding for anyone revisiting n>=7 weight-1 on this or similar hardware: it's a hard single-call memory ceiling, not a tunable-away leak -- moving past n=6 would need either more RAM or a fundamentally different approach (e.g. the Monte-Carlo-estimator fallback CONTEXT.md already hedges for once exact enumeration stops being tractable).
- **Plan 17-07 does not need this data.** Its own data-loading step already checks for the stretch CSVs and merges them in if present; their absence is expected and acceptable -- CORE data alone satisfies this phase's floor requirements.
