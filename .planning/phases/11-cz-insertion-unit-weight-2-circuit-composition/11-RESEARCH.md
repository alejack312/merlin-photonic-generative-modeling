...
The single most important and non-obvious finding: **directly wiring the module's own PBS dual-rail output into `heralded_cz`'s ctrl/data ports puts the CZ's `-1` phase on logical `|00⟩`, not `|11⟩`.** This was verified by actually running the composed circuit — `heralded_cz`'s internal bit convention is the *opposite* of this module's `H=(0,1)="0"`, `V=(1,0)="1"` PBS convention. The fix, also empirically confirmed, is to **swap each qubit's dual-rail pair order in the mode-mapping dict** (map polarization-carrying port `2k` to `heralded_cz`'s local slot 1, and vacuum-partner port `2k+1` to local slot 0) — after which the composed gate reproduces `diag(1,1,1,-1)` exactly on the module's own bit convention. This is exactly the kind of "self-consistent but backwards" bug the module has hit before (the Plan 09-02 H/V port-labeling correction) — flag it prominently in the plan and make the truth-table check (Success Criterion 1) the thing that pins it down, not an assumption.
...
### Pitfall 1: `heralded_cz`'s ctrl/data bit convention is inverted relative to this module's PBS convention (HIGH confidence — empirically verified by direct execution)
