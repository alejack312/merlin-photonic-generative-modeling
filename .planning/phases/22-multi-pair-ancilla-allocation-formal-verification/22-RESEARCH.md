...
## Open Questions (BOTH RESOLVED 2026-08-20 — see resolution notes inline)

> **Resolution status added after this file was written.** Both questions below were put to the owner and answered; the answers are recorded as D-05 and D-06 in `22-CONTEXT.md` and are enforced by the Phase 22 plans. This section is retained as the record of what was open at research time — read the resolution notes, not just the original text.

1. **(RESOLVED — see `22-CONTEXT.md` D-05, and Plan 22-03)** Is the round-robin edge-coloring allocation formula (A2) the scheme the owner actually wants, or is a different concretization of "pooled/recycled" intended?**
   ...
   - **RESOLVED:** the owner chose to reframe the model as a **search** question (find the minimum ancilla-block count K over K_n) rather than verify any fixed formula — `22-CONTEXT.md` D-05. The concretization checkpoint this recommendation asked for exists as **Plan 22-03**, exactly as scoped here (a bounded confirm-or-propose step, not a re-run of MPAIR-01). Plan 22-04 is explicitly forbidden from encoding the round-robin formula in the model; the formula lives only in the prose spec and the baseline, so the two can cross-check.

2. **(RESOLVED as scope, not as physics — see `22-CONTEXT.md` D-06, and Plans 22-01/22-02)** Does the deferred-post-selection architecture actually permit safe ancilla reuse across sequential gates at all (Pitfall 1 / A3)?
   ...
   - **RESOLVED — and the owner went further than this recommendation.** Rather than only documenting it as a scope boundary, the owner made it a **go/no-go gate on the whole phase** (`22-CONTEXT.md` D-06): Plan 22-01 answers it numerically (pooled-vs-dedicated comparison at a pre-committed `1e-9` decision rule, anchored so a harness bug is distinguishable from a real disagreement), and Plan 22-02 blocks on the owner's literal GO/NO-GO ruling with a genuine terminal NO-GO branch. If reuse turns out invalid, pooling is moot, contiguous allocation was right, and the phase stops and reports that. The scope-boundary documentation this recommendation asked for is *also* required, in the model header, the summary, and the MPAIR-06 docs section.
