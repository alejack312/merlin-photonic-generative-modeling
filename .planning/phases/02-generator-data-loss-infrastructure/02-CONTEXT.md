...
- Dimensionality: **start at 2**, matching the classifier's already-verified-working `input_size=2`. Latent dim and `QuantumLayer.input_size` are the same number in MerLin (confirmed via `PhotonicGenerator`/`NormalLatent` source) — there's no separate "noise dim" chosen independently of circuit size. `.simple()` caps `input_size` at 19 (`n_modes = input_size+1 ≤ 20`); bigger values mean richer encoding but real simulation-cost growth. No literature-standard value applies here (unlike classical GAN latent dims) because this is constrained by photonic simulation cost, not just expressiveness. Dimensionality is explicitly a Phase 3/4 tuning knob, not something Phase 2 needs to get "right" — 2 is a cheap, verified-safe starting point to unblock the July 25 end-to-end-run checkpoint.
