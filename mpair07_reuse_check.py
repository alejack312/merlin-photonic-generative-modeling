...
(iqp_photonic_encoding.py:576-646) to TWO sequential CP(alpha) insertions, and
compares:
...
DRAFTED verdict in results/phase22_reuse_gate.md. The owner rules at Plan
22-02's checkpoint.
"""
...
from iqp_photonic_encoding import allstate_iterator, _decode_single_qubit_pair
...
def build_two_gate_processor(n, pair_a, pair_b, thetas, alpha, mode="pooled"):
    ...
    (iqp_photonic_encoding.py:576-646) to TWO sequential CP(alpha) insertions
...
def postselected_distribution(proc, n, ancilla_modes, gate_qubits):
    ...
    (iqp_photonic_encoding.py:692-810), generalized to an arbitrary set of
