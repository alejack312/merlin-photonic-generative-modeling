...
[GitHub repo](https://github.com/alejack312/merlin-photonic-generative-modeling) · [Documentation review and reading guide](docs/documentation-review-2026-09-10.md)
...
**Trainability.** The exponential gradient-variance-decay signature is reproduced with no photonics at all by a closed-form classical null model for the tested circuit scopes. At the bandwidth used, the MMD kernel is numerically the identity matrix for `n<=4` (bandwidth smaller than the target grid's bin spacing), so the loss there reduces to plain squared-distance on a product distribution. At `n=5,6` the real kernel shows a genuinely non-negligible off-diagonal term, not the identity, yet the same classical null model still reproduces the shipped curve; this does not establish a circuit-independent mechanism. These data do not isolate a circuit-trainability contribution; they support the narrower null-model interpretation. There is no trainability finding here to report, positive or negative.

**Hardness under loss.** IQP's conjectured sampling hardness requires an entangling structure that grows with system size. The weight-1 circuit is a product distribution, and the current mixed circuit has only one fixed entangled pair plus independent qubits, so both current scopes are classically sampleable and neither is an asymptotic hardness candidate. Because this pipeline only reports shots where every photon survives, and among those shots the distribution is provably identical to the lossless one, **the TVD-vs-loss and anticoncentration-vs-loss curves are exact closed-form functions of the loss rate and circuit size, with zero circuit content**. The real result is conditional shape preservation and its throughput cost, not a new hardness proof; a denser, scaling entangling family would be required for that question. The expected number of samples needed for one usable shot grows exponentially with both circuit size and heralded-gate count. That cost is specific to the heralded-CZ construction's real ancilla photons. The project's other two-qubit gate, the tunable `CP(alpha)` (ARB-01), uses vacuum ancilla and avoids the extra ancilla-photon loss exponent, but still incurs gate postselection cost — see the hardness doc's 2026-09-03 scope correction.
...
The [bounded release summary](docs/v4-bounded-release.md) records the selected scope closure and clean-checkout verification. Main ring train TVD remains about 0.51–0.61; the n=9 sibling analytic references have TVD 0.85–0.99 and support validity 0.012–0.293. Their modeled cost for 20,000 accepted outcomes is approximately 1.4 × 10^15–7.0 × 10^16 source attempts. These are analytic estimates, not observed optical throughput. The original full scientific scope remains incomplete.

The v4.0 implementation adds a NumPy IQP trainer for the two-ring dataset, typed dataset and checkpoint contracts, the registered sibling inventory/replay boundary, a discrete-alpha/continuous-single-angle NAT control, Hamming-kernel metric panels, and a qualified photonic deployment path. Existing v1–v3 pipelines remain separate. The implementation and its provenance gates are exercised by the full suite; the exact current count, commands, hashes, and artifact paths are recorded in the [v4 correction-pass report](docs/v4-correction-pass-2026-09-10.md), [v4 milestone report](docs/v4-milestone-completion-report.md), and [requirement evidence ledger](.planning/v4-requirement-evidence.md).

The verified v4 physical scope is deliberately bounded: fixed-photon `g2=0`, explicit uniform per-photon loss, and final-only projection. Perceval controls pass for n=2/n=3 no-gate, single-gate-with-bystander, and shared-gate cases, with absolute accepted mass reported separately from conditional output quality. Registered n=4 spatial and Hamming ring smoke evaluations match their compiled references at floating-point precision; the n=6/n=8 training profiles are recorded, while larger-n photonic deployment is not claimed. The sibling `training_smoke` row, all four registered bandwidth cells, and all three registered Ghosh–Kim small-n cells are faithfully retrained from the pinned source trainer; the registered exact n=6 anti-concentration validation cell also has a byte-identical source-validation rerun. Large-n sampled, Qiskit-dependent, and unavailable-input rows retain their documented replay, reference-only, or blocked dispositions. NAT reports are matched ideal/model-derived continuations and do not establish noisy efficacy.

This milestone does not establish multiphoton `g2>0` behavior, a general intermediate-projection composition rule, larger-n photonic deployment, complete sibling retraining for rows with unavailable inputs, sampled-uncertainty extensions, or the unexecuted two-day n=4 stop-rule arm. The selected eight-cell sibling/substrate comparison is complete as a model-derived fixed-photon reference; it is not direct n=9 full-Fock or hardware evidence. Owner predictions for NULL-03–06 and the binding NULL-08 control are recorded and exercised within their declared scopes; NULL-07 remains an unadjudicated source-gap hypothesis, and the owner-authored WRITE-07 explanation is recorded with the implementation convention separately qualified. See the [v4 correction-pass report](docs/v4-correction-pass-2026-09-10.md) and [v4 completion report](docs/v4-milestone-completion-report.md) for the acceptance boundary rather than treating these gaps as interpretation-only limitations.
...
| Held-out MMD² (real-vs-real reference)          | 0.0114 (deterministic, no generator)                     |
