...
**Trainability.** The exponential gradient-variance-decay signature is reproduced with no photonics at all by a closed-form, no-circuit-content model. At the bandwidth used, the MMD kernel is numerically the identity matrix for `n<=4` (bandwidth smaller than the target grid's bin spacing), so the loss there reduces to plain squared-distance on a product distribution; at `n=5,6` the kernel is not literally the identity (a genuinely non-negligible off-diagonal term appears), but the same no-circuit-content model, using the real kernel, still reproduces the shipped curve. A weight-1 circuit has no entangling gate, so this cannot be attributed to circuit trainability. There is no trainability finding here to report, positive or negative.

**Hardness under loss.** First, a scope point that governs how to read everything else: IQP's conjectured sampling hardness comes from the correlations the ZZ (weight-2) terms create between qubits. Strip those out and hardness disappears completely — by construction, the weight-1 circuit is classically easy. Only the weight-2/mixed scope is a hardness candidate at all. **The TVD-vs-loss and anticoncentration-vs-loss curves are exact closed-form functions of the loss rate and circuit size, with zero circuit content**, because this pipeline only reports shots where every photon survives, and among those shots the distribution is provably identical to the lossless one. The real result: the *conditional* distribution (every photon detected) is provably identical to the lossless one at every tested loss rate — a statement about shape, not a new hardness proof; whatever hardness the lossless construction has (IQP's own conjectural argument, not established fresh here) carries over unchanged. What loss costs is throughput — the expected number of samples needed for one usable shot grows exponentially with both circuit size and gate count, not conditional hardness.
