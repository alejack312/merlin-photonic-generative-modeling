...
**Trainability.** The exponential gradient-variance-decay signature is a closed-form artifact of the loss metric: at the bandwidth used, the MMD kernel is numerically the identity matrix (bandwidth smaller than the target grid's bin spacing at every tested size), so the loss reduces to plain squared-distance on a product distribution — reproduced with no photonics at all. A weight-1 circuit has no entangling gate and cannot exhibit a barren plateau by construction. There is no trainability finding here to report, positive or negative.

**Hardness under loss.** First, a scope point that governs how to read everything else: IQP's conjectured sampling hardness comes from the correlations the ZZ (weight-2) terms create between qubits. Strip those out and hardness disappears completely — by construction, the weight-1 circuit is classically easy. Only the weight-2/mixed scope is a hardness candidate at all. **The TVD-vs-loss and anticoncentration-vs-loss curves are exact closed-form functions of the loss rate and circuit size, with zero circuit content**, because this pipeline only reports shots where every photon survives, and among those shots the distribution is provably identical to the lossless one. The real result: the *conditional* distribution (every photon detected) is exactly as hard as the lossless case at every tested loss rate, because it *is* the lossless distribution. What loss costs is throughput — the probability of getting a usable shot falls exponentially with both circuit size and gate count, not conditional hardness.
