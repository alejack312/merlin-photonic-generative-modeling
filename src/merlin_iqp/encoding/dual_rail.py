...
autograd -- instead of iqp_photonic.py's existing polarization
...
Bit convention (matches iqp_photonic.py's own, for direct
...
iqp_photonic.py's own build_weight2_processor does) and then
...
iqp_photonic.py's OWN `_build_weight2_processor_no_herald` +
...
def _bitstring_dist_from_layer_output(layer, out_flat, n):
    ...
    (reused, unmodified, from iqp_photonic.py), plus the residual
...
def build_dual_rail_weight2_processor(n, i, j, thetas):
    ...
    `_build_cz_insertion_core()` UNMODIFIED from iqp_photonic.py --
    ...
    iqp_photonic.py's established pattern."""
    assert len(thetas) == n
...
def dual_rail_weight2_input_state(n):
    ...
    (confirmed from scripts/heralded_cz_derisking.py: plain, unannotated 1-photon
...
def make_weight2_cp_circuit_and_input(n, i, j, alpha):
    ...
    iqp_photonic.py's own convention of rebuilding the circuit per
...
def dual_rail_photonic_cp_iqp_distribution(n, i, j, thetas, alpha):
    ...
    filter as the polarization version (reused logic, not re-derived):
      ...
      2. otherwise, qubit i's or j's own pair out-of-subspace ->
         ...
         bug iqp_photonic.py's own history already hit and fixed;
