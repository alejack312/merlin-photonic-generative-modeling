"""Tests whether list-adjacent outputs of ``NaturallyOrderedGenerator``
(under the natural/radius-sort ordering) are more sensitive to shared
parameter changes than random non-adjacent output pairs, via the output
Jacobian's row-wise cosine similarity."""
...
def compute_jacobian(gen, z: torch.Tensor) -> torch.Tensor:
    ...
    def f(p):
        """Functional forward pass under substituted parameters ``p``, routed
        through the ``.thetas`` monkey-patch (see the enclosing docstring)."""
