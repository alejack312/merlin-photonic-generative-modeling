"""The K=462 generator variant (GEN-07 checkpoint): output columns reordered
by Fock-state "smoothness" rank so they can be paired index-for-index with
bin centers sorted by ascending radius, replacing the K=400 baseline's
arbitrary index<->bin labeling."""
...
from merlin_iqp.generator.noise import LATENT_DIM
...
class NaturallyOrderedGenerator(nn.Module):
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        """Raw QuantumLayer output for latent batch ``z``, columns permuted
        into Fock-state "smoothness" order via ``self.perm``."""
