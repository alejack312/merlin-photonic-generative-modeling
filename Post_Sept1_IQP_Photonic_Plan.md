...
**Correction (2026-09-03):** that framing is imprecise and should be read alongside `docs/iqp-lit-scoping.md`'s own actual finding, not in place of it. KLM (dual-rail photons + heralded gates) already lets any qubit circuit, IQP included, be built in linear optics — that's a 2001 result, not an open question. What Phase 0's literature search actually confirmed unanswered is narrower: no paper studies a DV/Fock-space *IQP-specific* construction (analogous to what Douce et al. did for continuous-variable IQP) as its own subject. Separately, IQP itself is exactly non-adaptive X-Y-plane measurement on a graph state (Hoban, Wallman, Anwar, Usher, Raussendorf & Browne, "Measurement-based classical computation," PRL 112, 140505 (2014), arXiv:1304.2667) — not cited in the original Phase 0 search, and the more natural framing for a photonic realization given Quandela's own graph-state/fusion hardware direction. The open, non-trivial question this doc's "Phase 3" was actually trying to answer is a resource-cost comparison (KLM's heralded-gate approach vs. graph-state measurements, at equal photon budget, under loss) — not "can this be done at all."
